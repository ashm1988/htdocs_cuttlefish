...
<html lang="en">
  ...
  <body>
	...
	<div class="container">
			...
			#}
			$latest = 'current';
			$sql = "SELECT UPDATE_TIME FROM information_schema.tables WHERE TABLE_SCHEMA = 'versions_test' AND TABLE_NAME = 'current'";
			$result = mysqli_query($cxn, $sql)
				or
			die("Cant find update time");
			while($row = mysqli_fetch_assoc($result))
			{
				extract($row);
				$tbdate = $UPDATE_TIME;
	  ...
	  <div class="col-xs-4">
	    <form class="form-group"  action="new_versions.php" method="post" id="orderby">
	...
	<div class="container-fluid">
		<div class="col-xs-12">
			...
			'frapi' => 'Frapi Logging',
			'fix50' => 'Fix5 Logging',
			'fix50sp1' => 'Fix5SP1 Logging',
			'fix50sp1dc' => 'Fix5-DCSP1 Logging',
			'fix44' => 'Fix44 Logging',
			'fix44dc' => 'Fix44-DC Logging',
			);

			echo "Table Name: $latest<br />\n";
			echo "Updated: $tbdate<br />\n";
			...
			<th>$lables[core]</th>
			<th>$lables[otl]</th>
			<th>$lables[instance]</th>
			<th>$lables[licence]</th>
			<th>$lables[adapter]</th>
			<th>$lables[frapi]</th>
			<th>$lables[fix50]</th>
			<th>$lables[fix50sp1]</th>
			<th>$lables[fix44]</th>
			</tr></thread>\n";
